...
> 使用中收集的问题与迭代规划。已完成：模板两级模型/新建复制来源、HTTPS/config.json、历史版本回滚、响应式首轮与 Provider 卡片化等。

1. **视觉与交互细节跟进**：Provider 卡片/主题单图标/导航已初步统一；URL 页与模板页的按钮、卡片、图标风格需按新审美跟进；配色整体美化最后做。
2. **移动端/平板适配验证收尾**：断点约定手机 <768px、平板 768-1024px（主内容占满）、桌面 >1024px（80%）；各页面窄屏拥挤处实测微调。
3. **Monaco 控制台报错**：`[createInstance] ... UNKNOWN service ICodeLensCache`，偶发，待定位（暂不影响使用）。
4. **订阅下载可控性**：Provider 使用频率低暂缓；可加超时、自定义 User-Agent / 请求头。
5. **README 自身文档同步**：补 config.json/HTTPS、CLI `-d/--listen`、默认端口 9112、模板两级模型等说明。
6. **前端视觉样式整体美化**（最后处理）。
